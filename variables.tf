...
variable "enable_event_driven_replacement" {
  type        = bool
  default     = true
  description = <<-EOT
    Enable event-driven Spot replacement on ASG scale-out. When enabled,
    Xosphere immediately attempts to replace newly-launched On-Demand instances
    with Spot rather than waiting for the scheduled scan. Can be overridden
    per-ASG via the xosphere:instance-orchestrator:event-driven-replacement-enabled tag.
  EOT
}
